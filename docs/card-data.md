...
> 試遊版3勢力の75枚とリーダー3人が、すべてこの形式で表せることを確認済み（7章）。
...
type TargetSpec = {
  ...
  cellOwner?: "ally" | "enemy" | { ownerOf: string };  // マスの持ち主（{ownerOf} は「その対象ユニットの持ち主の盤面」）
...
試遊版3勢力の全カードとリーダーについて、この形式で表せるかを1枚ずつ確認した。表せないものはなかった。特殊な表し方をするものは次のとおり。
...
| レイ 成長後 | `swapCells`（同じプレイヤーの2マス。空きマスも可） |
...
| # | 項目 | 案 |
|---|---|---|
| 1 | 「使ったスペルの枚数」に、今まさに使っているスペル自身を含めるか | 含める【仮】（コストを払った時点で数える）。魔法の剣（AC-10）の強さに関わる |
