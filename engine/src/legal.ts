...
export function legalActions(cat: Catalog, state: GameState): Action[] {
  ...
  const affordable = (normal: number, flex: number) => r.paymentPlan(p, normal, flex) !== null;
  ...
  for (const inst of st.hand) {
    ...
    for (const enhance of def.enhance ? [false, true] : [false]) {
      if (!affordable(cost, enhance ? r.enhanceCost(p, def) : 0)) continue;
      if (def.type === 'unit') {
  ...
  st.leaders.forEach((l, idx) => {
    ...
    if (!affordable(0, r.leaderCost(p, idx))) return;
    for (const targets of r.targetCombos(ab.targets, p, true)) out.push(clean({ type: 'leaderAbility', player: p, leader: idx, targets }));
  });
  return out;
}
